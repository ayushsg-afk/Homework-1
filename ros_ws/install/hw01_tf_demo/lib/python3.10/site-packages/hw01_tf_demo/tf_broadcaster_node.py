...
# elementary rotation step sequence
STEP_SEQUENCE = [
    ("z", np.deg2rad(90)),
    ("x", np.deg2rad(90)),
    ("y", np.deg2rad(60)),
...
class Hw01TfBroadcaster(Node):
    ...
    def on_timer(self):
        ...
        # 1. Fetch current step rotation matrix
        axis, angle = STEP_SEQUENCE[self.step_index % len(STEP_SEQUENCE)]
        R_step = ELEMENTARY_ROTATIONS[axis](angle)

        # 2. Check composition frame mode parameter
        frame = self.get_parameter("compose_frame").value

        if frame == "current":
            # Right-multiply: compose relative to current body frame
            self.R_body = self.R_body @ R_step
        elif frame == "fixed":
            # Left-multiply: compose relative to fixed space frame
            self.R_body = R_step @ self.R_body
        else:
            self.get_logger().error(f"Unknown frame mode: {frame}")

        self.step_index += 1

        # Broadcast animated body frame
